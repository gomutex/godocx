...
func TestNumProp_MarshalXML(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			input: NumProp{
				ILvl:  &ctypes.DecimalNum{Val: 1},
				NumID: &ctypes.DecimalNum{Val: 42},
				NumChange: &ctypes.TrackChangeNum{
					ID:       123,
					Author:   "John Doe",
					Date:     &date,
...
func TestNumProp_UnmarshalXML(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: NumProp{
				ILvl:  &ctypes.DecimalNum{Val: 2},
				NumID: &ctypes.DecimalNum{Val: 5},
				NumChange: &ctypes.TrackChangeNum{
					ID:     1,
					Author: "author",
...
func compareTrackChangeNum(a, b ctypes.TrackChangeNum) bool {
	return a.ID == b.ID && a.Author == b.Author && *a.Date == *b.Date
}

func compareTrackChange(a, b ctypes.TrackChange) bool {
	return a.ID == b.ID && a.Author == b.Author && *a.Date == *b.Date
}
